...
function updateData(data, id) {
  return new Promise((resolve, reject) => {
    const sqlQuery = "UPDATE orders SET ? WHERE id = ?";
...
function getDataByName(name) {
  return new Promise((resolve, reject) => {
    const sqlQuery = "SELECT * FROM orders WHERE name = ?";
...
function getFieldsName() {
  return new Promise((resolve, reject) => {
    conn.query(`DESCRIBE orders`, function (error, result) {
      //      conn.query(`DESCRIBE orders`, function (error, result) {
      if (error) {
        reject(error);
      }
...
function getTotalData() {
  return new Promise((resolve, reject) => {
    conn.query(`SELECT COUNT(id) AS total_data FROM orders`, function (error, result) {
      //      conn.query(`DESCRIBE orders`, function (error, result) {
      if (error) {
        reject(error);
      }
